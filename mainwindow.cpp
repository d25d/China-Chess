...
{
    int ret = QMessageBox::question(this, tr("༼☯﹏☯༽"), tr("是否退出游戏？"), QMessageBox::Yes | QMessageBox::No);
...
{
    QMessageBox::about(NULL, tr("关于本软件"), tr("Chinese Chess Changed"));
}
...
void MainWindow::playTipSound(const MoveInfo &info, int gameResult)
{
    ...
    else
    {
        if (SRC(info.move) > 0 && DST(info.move) > 0)
        {
            ...
            {
                QSound::play(info.killedChessman == 0 ? AUDIO_MOVE : "");
            }
        }
        else
        {
